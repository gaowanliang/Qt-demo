...
static int isRgsOk(struct rgs_st *st);
...
static int isRgsOk(struct rgs_st *rcv)
{
	...
	if (ret == SQLITE_ROW)
		//find it
		rcv->rgsState = RGS_EXISTS;

	else if (ret == SQLITE_DONE)
	{
		rcv->rgsState = RGS_OK;
		snprintf(buf, BUFFERSIZE, "insert into rgsTable(count, password) values('%s', '%s')", rcv->cnt, rcv->pwd);
		puts(buf);
